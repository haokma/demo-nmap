...
def regex_version_id(ver_apa):
    ...
    for n in range(1, 12):
        ...
        req_get_response = requests.get(
            "https://www.cvedetails.com/version-list/45/66/{}/Apache-Http-Server.html?order=1".format(n))
        ...
        get_ver_id = re.findall(r'href="/vulnerability-list/vendor_id-45/product_id-66/version_id-....../Apache-Http-Server-{}.html"'.format(
            str(ver_apa)), str(req_get_response.content))
...
def nmap_scan_vul():
    ...
    if len(lst_ver_id) >= 1:
        for i in lst_ver_id:
            ...
            result = sc.scan(
                host, port, arguments=" --script /home/haonc/python_nma_nse/nmap_script.nse   --script-args ver_id={} ".format(ver_id))
